...
def credit_prediction_page(df):
    ...
    if st.button("Effectuer la prédiction"):
        try:
            ...
            prediction_response = requests.get(f'http://localhost:5000/predict/{st.session_state.sk_id}')
    ...
    if st.button("Obtenir le graphique SHAP"):
        if st.session_state.sk_id is not None:  # Utilisation de l'ID client de la session
            response = requests.get(f'http://localhost:5000/shap_plot/{st.session_state.sk_id}')
...
def get_prediction(sk_id):
    response = requests.get(f'http://localhost:5000/predict/{sk_id}')
