...
def plot_feature_importance(df):
    ...
    response = requests.get('http://127.0.0.1:5003/features')
...
def credit_prediction_page(df):
    ...
    if st.button("Effectuer la prédiction"):
        try:
            ...
            prediction_response = requests.get(f'http://127.0.0.1:5003/predict/{st.session_state.sk_id}')  # Correction ici
    ...
    if st.button("Obtenir le graphique SHAP"):
        if st.session_state.sk_id is not None:  # Utilisation de l'ID client de la session
            response = requests.get(f'http://127.0.0.1:5003/shap_plot/{st.session_state.sk_id}')  # Correction ici
...
def get_prediction(sk_id):
    response = requests.get(f'https://http://127.0.0.1:5003/predict/{sk_id}')
    ...
    if user == "yani" and password == "azerty":
        ...
        if uploaded_file is not None:
            ...
            response = requests.post(
                'https://http://127.0.0.1:5003/import/',
